<template>
    ...
    <ul class="list">
        <!-- 影片 -->
         <router-link :to='`../moviedetails/${item.filmId}`' class="item flex jc-sb pt-15" v-for="item in filmlist" :key="item.filmId"> 
            <div class="film flex jc-sa ml-10">
                <img :src="item.poster" class="image ">
                <div class="ml-10  lh15">
                    <p class="f16">{{item.name}} <span class="type">{{item.filmType}}</span></p>
                    <p class="f14 gray">观众评分 <span class="yellow">{{item.grade}}</span> </p>
